...
export function renderTray(ctx) {
  ...
  tiles.forEach(tile => {
    ...
    if (!isOnSlate) {
      const activate = () => ctx.onTileClick(tile, isRefused);
      el.addEventListener("click", activate);

      /* Keyboard reachability · WCAG 2.1.1 + 2.5.7 (Dragging Movements).
         Slating was mouse-only: a click handler on a non-focusable <div>, with
         drag as the other path. tray.js already treats tap-to-add as the
         equivalent of a drag, so the alternate path existed — it just could not
         be reached without a pointer. Exposing it costs one role + one key
         handler; no new interaction model.

         role=button rather than a real <button> because the tile's content is
         flow content (nested divs, a preview pane). Interactive elements only
         admit phrasing content, so wrapping this in <button> would be invalid
         markup — the composite-widget pattern is the correct one here. */
      el.setAttribute("role", "button");
      el.tabIndex = 0;
      el.setAttribute(
        "aria-label",
        `${isRefused ? "Refused source" : "Slate source"} · ${tile.label} · ${tile.source}`
      );
      el.addEventListener("keydown", (ev) => {
        if (ev.key !== "Enter" && ev.key !== " " && ev.key !== "Spacebar") return;
        ev.preventDefault(); /* Space would otherwise scroll the tray */
        activate();
      });
