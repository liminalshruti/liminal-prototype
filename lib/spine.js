...
/* Polarity classes for rendering. Upstream carries no presentation concern;
   ...
   Only the three epistemic-polarity edges are non-neutral, because only they
   name a measurement about a belief — which is what the v0.3 severity lane
   requires before a hue is spent. RECOMMENDS is structural routing, not a
   favorable finding, so it stays neutral (ink) despite sounding positive. */
