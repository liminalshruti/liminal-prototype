...
const BEAT_HANDLERS = {
  ...
  SEALING: (engine) => {
    ...
    if (artifact) {
      applySealChoreography(artifact, engine.sealChoreography, engine.sealRiseMs);
    }
...
function applySealChoreography(artifactEl, choreography, durationMs = 520) {
  ...
  switch (choreography) {
    ...
    case "bifurcated":
      ...
      if (typeof window.onOrbitalGlow === "function") {
        window.onOrbitalGlow();
      }
      break;
  }
}
