...
import { renderDivergence, resetDivergence } from "./divergence.js";
...
let engineState = {
  ...
  /* choreography parameters — Run-B adjudication (2026-07-02) */
  sealChoreography: "bifurcated",  // instant | rise | bifurcated · C1: master as richest ceremony
  sealRiseMs: 600,           // 360 | 500 | 520 | 600 · C1: bifurcated glow duration
  correctionModel: "rule-gate", // gloss-layer | rule-gate | visible-clause · C4: two patterns supported
  refusalFlavor: "routing",  // system | agent | routing · C5: three flavors documented
  refusalTimingMs: 320,      // 200 | 320 · C5: unified timing, cut 01/11 converged value
...
export function initLoopEngine(opts = {}) {
  ...
  if (opts.sealChoreography === undefined) engineState.sealChoreography = "bifurcated";  // C1: master ceremony
  if (opts.sealRiseMs === undefined) engineState.sealRiseMs = 600;  // C1: bifurcated glow
  ...
  if (opts.refusalTimingMs === undefined) engineState.refusalTimingMs = 320;  // C5: unified timing
...
const BEAT_HANDLERS = {
  IDLE: (engine) => {
    ...
    /* clear the divergence gate so the next cycle must view the split again */
    resetDivergence();
    const divergenceHost = document.getElementById("divergence-pane");
    if (divergenceHost) {
      divergenceHost.hidden = true;
      divergenceHost.innerHTML = "";
    }
  },
  ...
  DECIDING: (engine) => {
    /* user is making a disposition choice
       ...
       · This beat has been declared in the sequence since the engine was written
         but rendered nothing beyond relabelling the buttons. It now carries the
         divergence pane: where the three reads split, shown before the founder
         signs. Null-guarded on its host, same idiom as CAPTURING/READING — cuts
         without a #divergence-pane are unaffected. */
    renderAnnotations();
    setDispositionLabels(engine.subject);

    const divergenceHost = document.getElementById("divergence-pane");
    if (divergenceHost) renderDivergence(divergenceHost, makeCTX(engine));
  },
  ...
  SEALING: (engine) => {
    ...
    (async () => {
      try {
        ...
        if (vaultCountEl) {
          /* monotonic display: store count takes over only once it exceeds the
             narrative fixture count — sealing must never make the number drop */
          vaultCountEl.textContent = Math.max(realCount, vaultCount[activeProductId]);
        }
      } catch (err) {
