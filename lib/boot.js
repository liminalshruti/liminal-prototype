...
function wirePreviewSubject() {
  document.getElementById("preview-sub")?.addEventListener("click", () => {
    ...
    if (activeProductId === "team") {
      ...
      document.getElementById("modal-content-1").innerHTML =
        `<em>From: Kriti · Manager.</em> A coherence read on you was composed today. ` +
        ...
        `<strong>What it concluded</strong>: ${s.id === "brian_eng" ? "your pattern has narrowed, not broken · three open paths flagged · the manager has chosen to surface in Tuesday's 1:1" : s.id === "sean_cofounder" ? "stated-vs-observed in alignment · log-and-watch is the default · joint vault updated" : s.id === "maya_design" ? "attendance pattern stable · no further read · this is the lightest-weight register the system has on you" : "advisor channel · 6 weekly check-ins read for advisory framing"}. ` +
...
function boot() {
  ...
  const _hasEntered = sessionStorage.getItem("liminal-entered") === "1";
