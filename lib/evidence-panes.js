...
export const EVIDENCE_PANES = {
  /* spend · unit-safe workspace-credit reconciliation + control receipt (from cut 11)
     Renders: exact allocation/residual + source mismatch → guided-control verification */
  spend: {
    render: (host, ctx) => {
      /* spend evidence pane: conserved allocation + explicit source/control state */
      const allocHTML = `
        <div class="allocbar">
          <div class="p" style="width:91.43%">allocated 320K</div>
          <div class="s" style="width:8.57%">residual 30K<span class="ghost"></span></div>
        </div>`;

      const swapHTML = `
        <div class="swap">
          <div class="side cur">
            <span class="ro">source comparison</span>
            <div class="ag">500 / 494 / 501</div>
            <div class="co">contract / directory / vendor · pairwise +6 / -1 / -7 · no winner</div>
            <span class="bd">preserve mismatch</span>
          </div>
          <div class="arrow">→</div>
          <div class="side sug">
            <span class="ro">guided native control</span>
            <div class="ag">workspace group limit</div>
            <div class="co">applied_unverified · fresh export needed</div>
            <span class="bd">verify outcome</span>
          </div>
        </div>`;

      host.innerHTML = `<div class="evidence">
        <div class="ev-h">Evidence · workspace credit control<span class="tag">sanitized · 350K credits</span></div>
