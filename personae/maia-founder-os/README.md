...
- `snapshots/` — 6 synthetic events forming the demo session (.eml + meeting transcripts + PR thread + paper draft + application drafts)
- `corrections/trail.jsonl` — synthetic 9-tag correction stream
- `corrections/decisions.jsonl` — synthetic Tier 1/2 substrate-correction decisions
- `expected-vault-state.jsonl` — post-demo vault state for verification
...
The construction sequence (Apr 29-30) builds the snapshots, corrections, and demo script. This README + identity.json are the foundational character sheet that grounds everything downstream.
