...
|---|---|---|---|---|---|
| **B0 · BEACHHEAD** | An IT-operations director owns a 350K-credit hosted-AI pool, receives the overage in one cost center, cannot trust the vendor counts, and must establish a Finance-approved allocation before replacing blanket limits. | **Enterprise spend operator** | director · single operator + control owner | `11-govern` *(qualify source → retain 500 contract, 494 directory-derived, and 501 vendor-admin as independently accepted July observations with pairwise mismatches and no winner → allocate exactly → triage a 36K concentration finding → sign and guide one native workspace limit → require fresh after-state before verification)*. **Shows:** credits are not currency; source disagreement is not resolved by inference; application is not verification; the durable assets are the approved policy and receipted decision, not the dashboard. | live sanitized cut · production source of truth is `liminal-desktop` |
...
| **B2 · LAND** | Sam invites his manager; she becomes an operator at her node — audits/tweaks/governs the team's agent setup **without snooping**, sees the drift, hands an upward-reportable trail up with the new metric (errors caught, drift corrected). | **Sam → Rhea** | manager · operator + oversight | *(from B1)* **invite seam** *(Sam→Rhea handoff)* → `team-drift` *(Rhea reads the team's drift — incl. Tariq — rendered as a system to tune)* → `03-calibration` *(the record over time — the "errors caught / drift corrected" metric for the exec)*. **Shows:** governance is the SAME loop at the manager node; value flows to both at the invite edge. | refining · **GAP: the invite-seam and dedicated agent-setup governance cut** |
...
|---|---|---|---|
| **E1 · consent-gated reads** | Subjects opt into consent levels (pattern-only, summaries-only, calendar-only); the agent **refuses** reads above the level and names why. | Renders in `01-slate-tray` and `team-drift` — the tile shows "refused · out of consent." | live (the extracted CONSENT_CLASSES) |
| **E2 · observability without snooping** | Govern a team's AI usage without reading work content. | `11-govern` proves the metadata-only spend boundary; `team-drift` remains the agent-setup framing. | live framing · separate product cuts |
