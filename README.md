...
| `cuts/09-osint-custody.html` | **OSINT Custody — demonstration kernel.** Runs INGEST→READ→GUARD→REVIEW→VAULT in-browser via `lib/osint-kernel.bundle.js` (frozen demonstration kernel). Custody/DISCORD register toggle. Displays a **recorded** Kafka + Algorand provenance snapshot from `lib/osint-run.json` (current: localnet, not publicly verifiable) — see "OSINT Custody" §below. |
